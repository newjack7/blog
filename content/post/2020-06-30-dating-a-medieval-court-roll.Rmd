---
title: 'Dating a medieval court roll '
author: 'Jack Newman'
date: '2020-06-30'
slug: dating-a-medieval-court-roll
categories: []
tags: []
---
...
### Dating the roll
The previous post was spent learning the basics of text mining and exploring some possible avenues for analysis. In this post I label each offence with as precise a date as possible. In future I will also label it according to particular categories. I can do the first by extracting days, month, or years from the text to new columns and the pairing these with the original text. Essentially the text of the trials is broken into smaller observations and sorted according to particular criteria, in thie case, dates. This list is then appended to the original corpus which will allow the allegations to be sorted according to dates. The second will be achieved by assigning value to particular words which are indicative of particular activities. For example if I decide that the word 'feloniously' only ever refers to property or personal violence I can assign it a high value for those categories. This can then be applied to the text to give each offence a particular value. This post will focus on dating the allegations as accurately as possible by converting regnal years to calendar years and assigning precise dates where possible.

Initially, assigning years to each allegation is a fairly straightforward process. In some instances McLane, the editor of the roll, used calendar years '1327' etc. I quickly realised however that he also occasionally used regnal years which were not updated. Below is the plot of allegations which contain a calendar year. As before if you are new to R or coding please don't worry about understanding the code. I have supplied it so you can see my workings and in case it is of interest.
