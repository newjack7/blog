...
author: 'Jack Newman'
date: '2020-08-08'
slug: mining-a-medieval-court-roll-2-dating
categories: []
tags: []
...
plot1Year <- yearsCount %>% 
  ggplot(aes(word, n)) + 
  geom_col(fill="#ffd700", position = "dodge") + 
  geom_text(aes(label=n), hjust=-0.3, size=3.5) + 
  ggtitle("Frequency of allegations by year in TNA JUST 1/521") + 
  ylab(NULL) + 
  xlab(NULL) + 
  coord_flip() + 
  theme_minimal()
...
lincs_bigrams <- lincsEvents %>%
  unnest_tokens(bigram, word, token = "ngrams", n = 2)

#regYears <- lincs_bigrams %>% filter(bigram %in% c("edward 1", "edward 2","edward 3","edward 4","edward 5","edward 6","edward 7","edward 7","edward 8","edward 9","edward 10","edward 11","edward 12","edward 13","edward 14","edward 15"))

#In order to extract the regnal years however I need to split this text into two columns which I can filter
lincs_bigrams <- lincs_bigrams %>%
  separate(bigram, c("word1", "word2"), sep = " ")


#Filter for the regnal years
regYears <- lincs_bigrams %>% 
...
This creates a list of all occurrences of years from 1327 to 1341.

```{r echo=FALSE}
#Create table of all years mentioned in the text
yearsTag <- lincsEventsTokens %>% filter(word %in% c("1327", "1328", "1329", "1330", "1331", "1332", "1333", "1334", "1335", "1336", "1337", "1338", "1339", "1340", "1341"))

#Merge all years mentioned with the text
years <- left_join(yearsTag, lincsEvents)
...
plotRegnal <- regYears_count %>% 
  mutate(bigram = fct_relevel(bigram, '1 edward', '2 edward', '3 edward', '4 edward', '5 edward', '6 edward', '7 edward', '8 edward', '9 edward', '10 edward', '11 edward', '12 edward', '13 edward', '14 edward')) %>%  
  ggplot(aes(bigram, n)) + 
  geom_col(fill="#ffd700", position = "dodge") + 
  geom_text(aes(label=n), hjust=-0.3, size=3.5) + 
  ggtitle("Count of regnal years in TNA JUST 1/521") + 
  ylab(NULL) + 
  xlab(NULL) + 
  coord_flip() + 
  theme_minimal()
...
regYears_converted <-  regYears_united %>% 
  mutate(bigram = case_when(
    bigram == "1 edward" ~ "1327",
    bigram == "2 edward" ~ "1328", 
    bigram == "3 edward" ~ "1329", 
    bigram == "4 edward" ~ "1330", 
    bigram == "5 edward" ~ "1331", 
    bigram == "6 edward" ~ "1332", 
    bigram == "7 edward" ~ "1333", 
    bigram == "8 edward" ~ "1334", 
    bigram == "9 edward" ~ "1335",
    bigram == "10 edward" ~ "1336", 
    bigram == "11 edward" ~ "1337", 
    bigram == "12 edward" ~ "1338", 
    bigram == "13 edward" ~ "1339", 
    bigram == "14 edward" ~ "1340"
    ))
...
#yearsCombined <-  yearsCombined[!duplicated(yearsCombined$lincsID), ]

#Tags each entry with a regnal year

regnalTagged <- left_join(regYears_converted, lincsEvents)

regnalTagged <- regnalTagged %>% select(year, UUID, lincsID, regnalYear)
...
yearsCombined <- left_join(lincsEvents, yearsCombined, by = "UUID")
...
#This fixes errors which result from the formatting of the original text that had no space between the comma and the digits which refer to a date eg. '1,26' rather than '1, 26'. So when it was separated into trigrams it detected it as a single word rather than two words.
...
fullDates <- fullDates %>% filter(word3 !=  "shalâ")
